  ...
  <div class ="Extracurricular">
    ...
    <button class="bd1" v-on:click="addInput"><h1>Add Input</h1></button>
      <div class="inputArea" v-for="(input,k) in inputs" :key="k">
        <br>
        <div> Organization/Event:{{input.id}}</div>
        <button class="bd2" v-on:click="deleteInput(k)"><h1>Delete</h1></button>
        <hr>
        <!-- school -->
        <div>Organization/Event:</div>
         <textarea class="input" id="d1" rows="1" cols="100" onscroll="this.rows++;" v-model="input.school" placeholder="National University of Singapore" @mouseleave="getCode"></textarea>
         <br>
         <br>
        ...
        <!-- degree -->
        <div>Position:</div>
        <textarea class="input" id="d1" rows="1" cols="100"  v-model="input.degree" placeholder="Bachelor of Computting(information security)" @mouseleave="getCode"></textarea>
        <br>
        <br>
...
export default {
  ...
  methods: {
    ...
    this.$emit('extracurricularCode',this.outputCode);
